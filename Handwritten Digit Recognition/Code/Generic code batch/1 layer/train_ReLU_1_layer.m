...
% 
    ...
    for t = 1: epochs
        %for k = 1: batchSize
            ...
            hiddenActualInput = hiddenWeights*inputValues;
            hiddenLayerOutput = activationFunction(hiddenActualInput);
            outputActualInput = outputWeights*hiddenLayerOutput;
            outputLayerOutput = activationFunction(outputActualInput);
            
            %targetVector = targetValues(:, n(k));
            
            % Backpropagate the errors.
            outputDelta = dActivationFunction(outputActualInput).*(outputLayerOutput - targetValues);
            hiddenDelta = dActivationFunction(hiddenActualInput).*(outputWeights'*outputDelta);
            
            outputWeights = outputWeights + learningRate.*outputDelta*hiddenLayerOutput';
            hiddenWeights = hiddenWeights + learningRate.*hiddenDelta*inputValues';
        %end;
...
%        error = error/batchSize;
    ...
    E = outputLayerOutput - targetValues;
    error = mean(mean(E.^2));
    plot(t, error,'*');
    end;
 end