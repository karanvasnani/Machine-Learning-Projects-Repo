...
Val_targetValues = 0.*ones(10, size(labels_val_karan, 1));
for n = 1: size(labels_val_karan, 1)
    Val_targetValues(labels_val_karan(n) + 1, n) = 1;
end;
...
[hiddenWeights, outputWeights, error, correctlyClassified, classificationErrors] = train_ReLU_1_layer(activationFunction, dActivationFunction, numberOfHiddenUnits, norm_imgs_train_karan, labels_train_karan, targetValues, epochs, batchSize, learningRate);
...
[correctlyClassified, classificationErrors] = val_ReLU_1_layer(activationFunction, hiddenWeights, outputWeights, norm_imgs_val_karan, labels_val_karan,Val_targetValues);
